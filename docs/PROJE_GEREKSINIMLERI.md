...
2. [Domain Analizi: Telekom CRM Nedir?](#2-domain-analizi-telekom-crm-nedir)
3. [Aktörler ve Persona Tanımları](#3-aktörler-ve-persona-tanımları)
...
6. [MVP Kapsamı (Scope In / Scope Out)](#6-mvp-kapsamı)
7. [Microservices Mimarisi — Servis Bazlı Bounded Context](#7-microservices-mimarisi)
8. [Servis Detayları](#8-servis-detayları)
9. [Servisler Arası İletişim ve Event Akışı](#9-servisler-arası-iletişim-ve-event-akışı)
10. [Veri Modeli — Yüksek Seviye Varlıklar](#10-veri-modeli--yüksek-seviye-varlıklar)
11. [Teknoloji Yığını](#11-teknoloji-yığını)
12. [API Tasarım Standartları](#12-api-tasarım-standartları)
13. [Güvenlik Mimarisi](#13-güvenlik-mimarisi)
14. [Kabul Kriterleri (Acceptance Criteria)](#14-kabul-kriterleri)
...
**Sorumluluk:** Tarife, addon, VAS ürünlerinin master katalog yönetimi. Read-heavy servis — Redis cache yoğun kullanılır.
...
**Sorumluluk:** Sipariş alma ve Saga ile orchestrate etme. Customer → Catalog → Subscription → Payment zincirini yönetir.
...
**Sorumluluk:** Abonelik state machine'ini yönetir. MSISDN allocation/release.
...
**Sorumluluk:** CDR (Call Detail Record) eventlerini tüketip kullanım sayaçlarını günceller. Write-heavy.
...
**Sorumluluk:** Aylık bill-run scheduler ve fatura üretimi.
...
**Sorumluluk:** Ödeme alma ve PSP entegrasyonu (mock).
...
**Sorumluluk:** Çok kanallı bildirim gönderimi.
...
- Consume: neredeyse tüm domain eventler (template tabanlı eşleştirme)

### 8.9 Ticket Service

**Sorumluluk:** Müşteri talep / şikayet yönetimi, SLA.
...
4. Subscription Service consume PaymentCompleted
   -> allocate MSISDN
   -> create Subscription
   -> SubscriptionActivated  ===> Kafka
5. Order Service consume SubscriptionActivated
   -> mark order FULFILLED
6. Notification Service consume SubscriptionActivated
   -> send welcome SMS

Kompansasyon: Eğer SubscriptionActivation fail olursa
   -> Subscription Service: SubscriptionActivationFailed
   -> Payment Service: refund tetiklenir
   -> Order Service: order CANCELLED'a düşer
```
...
2. KYC belgesi yüklenir ve admin tarafından onaylanır.
3. Müşteri postpaid bir tarife seçip sipariş verir.
4. Mock PSP ile ödeme başarılı olur.
5. Subscription otomatik aktive olur, MSISDN atanır.
6. Müşteriye welcome SMS (mock log) gider.

### 14.2 Senaryo: Aylık Fatura

1. Bill-run job manual tetiklenir.
2. Aktif abonelerin son 1 aylık usage'i agregate edilir.
3. Her abone için invoice oluşur, PDF üretilir.
4. `InvoiceGenerated` eventi ile notification servisi e-posta atar.
5. Müşteri faturasını ödediğinde `InvoicePaid` eventi tetiklenir.

### 14.3 Senaryo: Kota Aşımı

1. CDR simulator usage event'leri üretir.
2. Usage service kotaları azaltır.
3. %80'de uyarı SMS'i gider.
4. %100'de ek paket önerisi içeren SMS gider.
5. Aşım sonrası kullanım billing'e overage olarak gider.
